...
public class SplashScreenPlugin extends CordovaPlugin {
    ...
    protected void pluginInitialize() {
        ...
        if (!showSpinner && !hasCustomSplashscreens) {
            ...
            behaviours.registerBehaviour(new AndroidSplashScreenBehaviour(autoHide, delayTime, isFadeEnabled, fadeDuration));

        } else {
            // Using the Android Splashscreen API is mandatory, so we'll try to use it as least as possible
            behaviours.registerBehaviour(new AndroidSplashScreenBehaviour(true, 1, false, 0));
    ...
    private static class AndroidSplashScreenBehaviour implements SplashScreenBehaviour {
        ...
        public AndroidSplashScreenBehaviour(boolean autoHide, int delayTime, boolean isFadeEnabled, int fadeDuration) {
            // Configuration variables
